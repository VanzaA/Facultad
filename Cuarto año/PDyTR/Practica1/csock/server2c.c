...
int main(int argc, char *argv[])
{
    ...
    if (n < 0) error("ERROR reading from socket");
    printf("Checksum de client: %lu  Mi checksum %lu\n", buffer_checksum, hash(buffer));

    if(strlen(buffer) == data_length){
        printf("El tamaño recibido y el tamaño enviado por el cliente coincide y es %lu \n",strlen(buffer));   
    }
    else{
        printf("No llegaron todos los datos, solo llegaron %lu \n", strlen(buffer));
    }    
